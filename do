...
10.
  I have functional sntrup761x25519sha512, now let's make a crypto/post_quantum/ChaCha20-Poly1305/mod.rs and traffic/mess_test_without_auth.rs with a client func that will encrypt the arg mess and send it, and a server one that will println! it, but after decrypting make sure that it is just safe non-injection-like max u8 length text and then println! it, then implement the     //macro run functions from the traffic dir that have the same name as is the name of the key in the Args struct that is so far refferenced only as a comment in run_logic_based_on_args in client/mod.rs, i will put that macro in utils/macros.rs to the other ones, then implement a server/traffic.rs file that will have a traffic() func that will be called in server/mod.rs , that will for now have handled only using of the server mess_test_without_auth func
